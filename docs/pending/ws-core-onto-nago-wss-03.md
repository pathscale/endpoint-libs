# `ws-core`, `ws` and `ws-client` onto `nago-wss`: 03

Date: 2026-09-22
Revision: 2. The state table names the Date commit. The roll commit is the
one this sentence is in, and it is the only difference. Successor of
`ws-core-onto-nago-wss-02.md`. Completed work is one line plus the proof
commit. Do not copy line numbers out of 02.

## How this series works

This file is the queue. Work the highest number. When section 6 says to roll,
write `ws-core-onto-nago-wss-04.md` as a complete document, banner this file
...
A **DECISION** item may not be started as code. P0.3, P0.4 and P0.5 are
settled. P0.1 is only half done. P0.2 is open. Lane 3 has not been started.

## 1. Where this sits

| Bucket | Location | Rule |
| --- | --- | --- |
| 1 | this file | Blocking for the port. Rolls to `ws-core-onto-nago-wss-04.md`. |
...
| --- | --- | --- | --- | --- |
| `~/code/endpoint-libs` | `fix/ws-core-lane1` | `9f43c32` | 3.2.0 | clean, apart from this file when it is the edit in progress |
| `~/code/nagoya` | `feat/resolve` | `e0ff612` | 0.1.10 | clean |
| `~/code/nago-wss` | `feat/nagoya-resolve` | `6e52935` | 0.2.0 | clean |
| `~/code/nago-rustls` | `master` | `91a55f0` | 0.1.1 | clean |

The endpoint-libs row is the Date commit. The commit that adds this file sits
directly on it and changes only the queue, the decision follow-up and the
TODO pointer, so `git rev-parse` reads one commit ahead of that row.

`feat/resolve` is `fix/taskset-push-order` at `a0dd8ad` plus the resolver
commit. That older branch is unchanged. `feat/nagoya-resolve` is
`bench/nagoya-write-path` at `3d069ed` plus the caller commit. The bench
branch is unchanged. Neither new branch may be merged while nago-wss
path-depends on nagoya or nago-rustls.
...
`resolve` / `connect_any`, which 0.1.9 does not have. Cite those functions
only against `feat/resolve`. `Notify::notify_waiters` is on the pinned 0.1.9.

**Do not give endpoint-libs a path dependency on the sibling to finish
P0.1.** A path dependency must not reach master, and this branch is the one
that would carry it there. The caller lands when a published nagoya contains
the functions.
...
`nago-rustls` and `nagoya`. Its lock now records nagoya 0.1.10 for that path.
...
`./scripts/check-features.sh` with `CHECK_CMD="cargo check --offline --lib"`
passed on `9f43c32`. The `--all-targets` form was not re-run.
...
- `tokio::select!` is gone under `src/`. The replacement is
  `futures::future::select`, left-biased. The session select lives in a block
  that ends before the arms touch `self`, because `pin_mut` holds the borrow
...
- nagoya has no unix signal module. `struct Signal` in `block_on.rs` is the
  parker's waker.
- On the kqueue poller, `wait` ignores `EVFILT_USER` and pushes every other
  event as fd readiness. An `EVFILT_SIGNAL` event would be pushed with
  `readable` and `writable` both false. `Registration` is an fd: `add` takes
  an fd and `Drop` calls `remove(fd)`. `signalfd` fits that. `EVFILT_SIGNAL`
  does not, until registration can name a signal number.
- `EVFILT_SIGNAL` does not replace the default disposition. `SIGINT` and
  `SIGTERM` still terminate the process unless the disposition is `SIG_IGN`
  or a handler. The filter records the signal even when it is ignored. A
  signal in the gap between the two calls is lost.
- `signalfd` delivers a signal only once it is blocked. The mask is
  process-wide.
...
3. **Replace `CancellationToken`.** `5d6ae8e`. The flag is `Shutdown`. Unix delivery is still `tokio::signal`.
4. **Teardown and policy-close.** `1a35b49`. `Outbound::Closed` and `WsStreamState::end`.
5. **`TOOLBOX` per poll.** `b55e62b`. `thread_local`, not `scoped-tls`.
6. **Drop `Date`.** `9f43c32`. `cached_date`, the spawn, the trait parameter and the header are gone. `ws-core` no longer names `tokio/time` or `httpdate`. Under the feature set karen uses, tokio's features are `net`, `rt`, `sync`, `signal`.
...
- **P0.2. Signal delivery in nagoya.** `EVFILT_SIGNAL` / `signalfd` are
  unwritten. New `unsafe` reactor code. Settled by a waiter for `SIGINT`,
  `SIGTERM` and `SIGHUP` without tokio. The flag half does not wait on it.
  See the poller notes in section 3 before writing it. Item 12 waits on this.
...
10. **Outbound path.** P0.3 settled this: the queue stays, owned here, bounded, `try_send` `Full` and `Closed`, not a third channel crate and not `futures` (a slot per sender would move `drop_conn_on_buffer_full`). `Outbound::Closed` and `WsStreamState::end` stay. Unblocked. Not started.
...
12. **`signal` delivery, after P0.2.** Rewrite `libs/signal.rs` against nagoya. Drop `tokio/signal` from the `signal` feature. The `Shutdown` flag stays.
13. **`ws-client`.** `connect_plain` and `connect_secure` cover the H1 client. `connect_h2` stays on hyper (P0.5).

### Lane 4

14. **`ws-core` stops naming `dep:tokio`.** Not true on `9f43c32`. The karen tree still prints tokio.
...
- Lane 1 and item 6 are on `fix/ws-core-lane1`, not merged, and not on PR #52.
- P0.3, P0.4 and P0.5 have rows in `docs/decisions/ws-core-onto-nago-wss.md`. P0.1 and P0.2 do not.
- The two smuggled edges have a test each, on `1a35b49`.
- Under the feature set karen uses, `cargo tree -e normal -i tokio` prints tokio (`net`, `rt`, `sync`, `signal`). P0.5 kept hyper on `ws` / `ws-client`, which karen does not enable. The remaining tokio is the TCP path, the queues and signal delivery.
- `check-features.sh` passed on `9f43c32` with `cargo check --offline --lib`. `check-chain.sh` did not.
- No path dependency on nago-wss, nago-rustls or nagoya has reached master. The nago-wss branch still has its path dependencies.
...
item is P0.2. Item 10 is unblocked and does not wait on it. Do not start
item 12 while P0.2 is open, and do not start item 11's H2 half.
...
**Roll this file** when P0.2 is settled, or when item 10 lands. The successor
...
Line numbers in 02 were read at older HEADs. Re-derive a line from the
